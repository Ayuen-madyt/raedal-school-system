...
// import promoteAllStudents from "../../api/promote";
...
const Layout = ({ children }) => {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:8001/api/settings/get")
      .then((res) => {
        localStorage.setItem("currency", res.data.settings.currency);
      })
      .catch((error) => {});
    axios
      .get("http://localhost:8001/api/users/user/1")
      .then((res) => {
        if (res.data.status === "") {
          router.push("/login");
        }
