...
export default function StaffInfo({ headerTitle, mode, staff }) {
  ...
  const [secondName, setSecondName] = useState(staff?.secondName);
  const [lastName, setLastName] = useState(staff?.lastName);
  const [title, setTitle] = useState(staff?.title);
  const [state, setState] = useState(staff?.state);
  const [passport, setPassport] = useState(staff?.passport);
  ...
  const postData = () => {
    ...
    axios
      .post(`http://localhost:8001/api/staff/post`, formData, {
        headers: {
  ...
  const deletestaff = () => {
    ...
    axios
      .delete(`http://localhost:8001/api/staff/delete/${staff?._id}`)
      .then((res) => {
        setTimeout(() => {
        ...
        <div className={classes.inner}>
          ...
          <Group>
            <Group ml={5} spacing={5} className={classes.links}>
              ...
              <Button color="red" compact onClick={deletestaff}>
                {waitingDelete ? (
      ...
      <Center>
        ...
        >
          ...
          >
            <Title
              style={{ fontSize: "15px" }}
            >{`${staff?.title} ${staff?.firstName} ${staff?.secondName}`}</Title>
